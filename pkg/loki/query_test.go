...
func TestFlowQuery_AddLabelFilters(t *testing.T) {
	...
	cfg := NewConfig(lokiURL, lokiURL, time.Second, "", "", false, "", false, ".*-ingress$", []string{"foo", "flis"})
...
func TestQuery_BackQuote_Error(t *testing.T) {
	lokiURL, err := url.Parse("/")
	require.NoError(t, err)
	cfg := NewConfig(lokiURL, lokiURL, time.Second, "", "", false, "", false, ".*-ingress$", []string{"lab1", "lab2"})
	query := NewFlowQueryBuilderWithDefaults(&cfg)
