...
// DefaultNodeInner: slightly modified from @patternfly/react-topology/src/components/nodes/DefaultNode.tsx
...
const DefaultNodeInner: React.FunctionComponent<DefaultNodeInnerProps> = observer(
  ({
    ...
    const refs = useCombineRefs<SVGEllipseElement>(
      hoverRef as React.Ref<SVGEllipseElement>,
      dragNodeRef as React.Ref<SVGEllipseElement>
    );
    ...
    const statusDecorator = React.useMemo(() => {
      ...
      const decorator = (
        <Decorator
          ...
          ariaLabel={status}
          innerRef={decoratorRef as unknown as React.Ref<SVGGElement> | undefined}
        />
      );
