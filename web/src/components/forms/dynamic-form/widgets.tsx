...
import { Checkbox, Dropdown, DropdownItem, DropdownToggle, Flex, FlexItem, Switch } from '@patternfly/react-core';
...
export const SelectWidget: React.FC<WidgetProps> = props => {
  ...
  return (
    <Dropdown
      ...
      toggle={
        <DropdownToggle id={`${id}-toggle`} onClick={() => setIsOpen(!isOpen)} onToggle={() => setIsOpen(!isOpen)}>
    ...
    >
      {enumOptions.map(option => (
        <DropdownItem
          ...
          value={option.value}
          onClick={() => {
            onChange(option.value, undefined, id);
            setIsOpen(false);
          }}
        >
          {option.label}
