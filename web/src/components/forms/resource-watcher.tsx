...
import { k8sCreate, k8sDelete, k8sUpdate, useK8sWatchResource } from '@openshift-console/dynamic-plugin-sdk';
...
import { Error as ErrorComponent } from '../messages/error';
...
export type ResourceWatcherContext = {
  ...
  data: any;
  onSubmit: (data: any, isDelete?: boolean) => void;
  loadError: any;
  errors: string[];
...
export const ResourceWatcher: FC<ResourceWatcherProps> = ({
  ...
  const [cr, crLoaded, crLoadError] = useK8sWatchResource<any>(
    name
      ? {
          groupVersionKind: {
            group,
  ...
  const data = cr
    ...
    ? exampleForModel(
        matchingCSVs.items.find(csv =>
          csv.spec.customresourcedefinitions?.owned?.some((crd: any) => crd.kind === kind)
        ),
        group,
        version,
        kind
