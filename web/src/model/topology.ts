...
import { HealthStat, HealthStats } from '../components/health/health-helper';
...
const getResourceStatus = (health?: HealthStat): NodeStatus => {
  ...
  if (health.critical.firing.length > 0) {
    return NodeStatus.danger;
  }
  if (health.warning.firing.length > 0) {
    return NodeStatus.warning;
  }
  if (health.other.firing.length > 0) {
    return NodeStatus.info;
  }
