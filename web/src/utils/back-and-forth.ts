...
import { setTargeteableFilters } from './filters-helper';
import { mergeStats, substractMetrics, sumMetrics } from './metrics';

export const getBackAndForthFetch = (filterDefinitions: FilterDefinition[], filters: Filters, matchAny: boolean) => {
  // check back-and-forth
  if (filters.list.some(f => f.def.category === 'bnf')) {
    ...
    let dstList = setTargeteableFilters(filterDefinitions, filters.list, 'dst');
    if (matchAny) {
      // In match-any mode, remove non-swappable filters as they would result in duplicates
      dstList = dstList.filter(f => f.def.id.startsWith('src_') || f.def.id.startsWith('dst_'));
    }
    return {
